...
[![Release](https://img.shields.io/github/v/release/Nuvraxis/taskQ?sort=semver)](https://github.com/Nuvraxis/taskQ/releases/latest)
...
[![lint: golangci-lint](https://img.shields.io/badge/lint-golangci--lint-00ADD8)](.golangci.yaml)
...
![taskQ package architecture](taskQ_package_architecture.png)
...
A few other Go task queues exist; here's how taskQ differs, stated as
plainly as I can manage about my own project.

| | taskQ | [`vmihailenco/taskq`](https://github.com/vmihailenco/taskq) | [`oshankkumar/taskqueue-go`](https://github.com/oshankkumar/taskqueue-go) |
| --- | --- | --- | --- |
| Payload type safety | Generics: `Task[T]`/`Handler[T]`/`Queue[T]`, compile-time checked | Reflection-based task registry, string task names | Concrete `*Job` with a JSON payload |
| Backends | In-memory, Redis Streams, Postgres (`SKIP LOCKED`) | Redis, SQS, IronMQ, in-memory | Redis (Postgres/MySQL job-store on the roadmap, not shipped) |
| Crash recovery | Built into every backend (`XAUTOCLAIM`, Postgres lease expiry) | Backend-dependent (SQS/IronMQ visibility timeout) | Redis Lua-script atomic dequeue plus visibility timeout |
| Cross-backend conformance | One shared suite (`taskqtest`) every backend must pass | Not present | Not present (single backend today) |
| Middleware | `Chain()` composition: logging, panic recovery, OpenTelemetry | Fallback handler for failed messages | Not documented |
| Delayed / scheduled delivery | Not yet (roadmap item) | — | Built in |
| Multi-step workflows / sagas | Yes, via `Saga[S]` with automatic reverse-order compensation | No | No |
| Web dashboard | No | No | Yes: queue/job monitoring, retry/delete from the UI |
| Maturity | New, pre-1.0 | ~1.3k GitHub stars; production use (built by the Uptrace team) | Early-stage, small |
| License | MIT | BSD-2-Clause | MIT |

taskQ's actual bets: generics for payload safety instead of a reflection-based
registry, a real Postgres backend that neither alternative has, one
conformance suite that keeps backends genuinely interchangeable rather than
each growing its own quirks, and sagas. None of the alternatives above
support multi-step compensating workflows. In exchange, taskQ is newer and
less battle-tested than `vmihailenco/taskq`, has no web dashboard the way
`oshankkumar/taskqueue-go` does, and doesn't yet support delayed/scheduled
delivery natively (it's on the roadmap).
...
| Type | Role |
| --- | --- |
| `Broker` | The backend contract: `Enqueue`, `Dequeue`, `Ack`, `Nack`. Deliberately minimal. Backend-specific behavior (delays, priority, and so on) lives in constructor options, not the interface. |
...
- **`Ack`** means the task is done and should be removed, whether it
  succeeded or the caller gave up after exhausting retries.
...
needs no setup; the Redis and Postgres sets need a running server (see each
backend's README):
...
| --- | --- | --- |
| `WithKeyPrefix(p)` | `"taskq:"` | Prefix for stream keys, for namespacing on a shared Redis. |
...
timeout) until it's Acked or the lease expires. That's how it recovers a
...
Bring up a local Postgres (and Redis, for `redisbroker`) with:

```sh
make deps-up   # docker compose up -d, redis:7 + postgres:16, credentials taskq/taskq/taskq_test
```
...
- **PgBouncer** in transaction-pooling mode doesn't forward `LISTEN/NOTIFY`, so every wake-up degrades to the poll interval (this doesn't affect correctness, only latency).
- **Admin methods** beyond the `Broker` interface: `QueueDepth`, `QueueDepthAvailable`, `PeekMessages`, `ReapExpiredLease`, `PurgeQueue`, `ListQueues`, `PurgeAll`, for monitoring and cleanup. `ListQueues` and `PurgeAll` are table-wide and refuse to run on a `Broker` constructed with `WithQueuePrefix` (they'd otherwise leak or delete rows outside that Broker's own namespace). Use `PurgeQueue` for a single queue instead, or a `Broker` with no prefix.
...
- **The retry budget is per hop.** `WithMaxRetry(n)` (default 3) applies to
  each individual `Do` or `Compensate` call, using the same
  `MaxRetry + 1`-attempts semantics as the rest of taskQ, not the run as a
  whole.
- **Hooks report the real outcome.** `WithOnComplete` fires once every step's
  ...
  `Compensate` itself exhausts its retries. That's the one outcome a saga
  can't resolve on its own, and it leaves the run partly rolled back. Treat
  it as page-a-human, not log-and-continue.
...
CI runs build/vet/tidy and `go test -short` across Linux, macOS, and
Windows; the race detector on Linux and macOS only (Windows lacks a
reliable CGO toolchain in the runner image); and `golangci-lint`,
`govulncheck`, and the `taskqtest` conformance suite (against live Redis
and Postgres service containers) on Linux.
